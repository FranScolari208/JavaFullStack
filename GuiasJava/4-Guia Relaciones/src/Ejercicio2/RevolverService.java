...
public class RevolverService {
    ...
    public Revolver llenarRevolver(){
        ...
        Integer numeroActual = (int)(Math. random()*6+1);
        Integer numeroDeDisparo = (int)(Math. random()*6+1);
        
        pistola.setPosicionActual(numeroActual);
        pistola.setPosicionDeBala(numeroDeDisparo);
        System.out.println(pistola.toString());
        return pistola;

    }
    public Boolean mojar(Revolver pistola){
        return pistola.getPosicionActual().equals(pistola.getPosicionDeBala());

    }
    
    public void siguienteChorro(Revolver pistola){
        Integer siguienteChorro;
        if(pistola.getPosicionActual()+1 > 6){
            
            siguienteChorro = 1;

        }else {
            siguienteChorro = pistola.getPosicionActual()+ 1;
        }
        
        pistola.setPosicionActual(siguienteChorro);
    }
    
    public void darDetalle(Revolver pistola){
        System.out.println(pistola.toString());
    }
}
